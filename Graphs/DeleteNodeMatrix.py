...
def DeleteNode(g: Graph, DeletedNode: str):
  
    if DeletedNode in g.nodes:
        index = g.nodes.index(DeletedNode)
        g.nodes.remove(DeletedNode)
        g.adj_matrix.pop(index)
    for i in range(len(g.adj_matrix)):
      g.adj_matrix[i][index] = 0
